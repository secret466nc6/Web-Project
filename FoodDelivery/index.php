...
echo "<p>現在時間：".$datetime."</p>";//可換時鐘API
$sqlOrder = "SELECT * FROM `OrderTable` WHERE `MenuArrive` != 1";
...
if($numOrder>0){
        ...
        if(isset($_SESSION['email'])){
             echo "Hi! ".$row['MemberName']."，";
        }
          echo '目前有'.$numOrder.'人在揪團唷!</strong> 馬上加入訂購美食~
              </div>';
}else{
    ...
    echo '目前沒人在揪團…</strong> 馬上號召訂購美食~</div>';
}
...
$resultCount = mysql_query($sqlCount);
     ...
     if($numCount>0){
           echo  '<a class="btn btn-warning" href="detail.php?OrderID='.$rowOrder['OrderID'].'&MemberID='.$MemberID.'" role="button">修改訂單 &raquo;</a></p></div> </div>';
          }else{
          echo  '<a class="btn btn-default" href="detail.php?OrderID='.$rowOrder['OrderID'].'&MemberID='.$MemberID.'" role="button">我要加入 &raquo;</a></p></div> </div>';
          }
